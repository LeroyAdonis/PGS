...
export function Step2SocialConnect({ onNext, onBack, businessProfileId }: Step2SocialConnectProps) {
  ...
  const handleConnect = async (platform: string) => {
    ...
    setPlatforms((prev) =>
      prev.map((p) => (p.platform === platform ? { ...p, connecting: true } : p))
    )
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPlatforms((prev) =>
          prev.map((p) => (p.platform === platform ? { ...p, connecting: false } : p))
        )
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground mt-2">
          Link your social media accounts to start publishing content automatically. You can connect
          more accounts later in your settings.
        </p>
      </div>
